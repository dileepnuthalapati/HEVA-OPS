...
42. **Push Crash Root Fix + Ghost User Fix (Apr 16, 2026)** — (1) REMOVED auto push notification init from login entirely. Push is now opt-in via "Enable Notifications" button in Settings > Security. Added crypto.randomUUID fallback for older Android WebViews. (2) Staff deletion now cascade-closes open attendance records (flag_reason=user_deleted). Dashboard filters deleted users from on-shift count. New admin endpoint POST /api/attendance/force-close-stale for orphan cleanup.
