...
52. **Monorepo Web Deployment (Apr 21, 2026)** — Enabled browser access on `app.hetupathways.com`. (1) Added static-file serving to `server.py`: when `/app/frontend/build/index.html` exists, backend mounts `/static` for hashed JS/CSS and serves `index.html` as the SPA fallback for any non-API / non-template route. Catch-all explicitly excludes `/api/`, `/socket.io/`, `/menu/`, `/kds-monitor/`, `/static/` so those routes are never shadowed. When the build folder doesn't exist (local dev) the block is silently skipped — dev mode on port 3000 is unchanged. (2) Updated `railway.toml`: build now runs `yarn install --frozen-lockfile && REACT_APP_BACKEND_URL='' yarn build` before installing backend deps. Empty `REACT_APP_BACKEND_URL` makes the web bundle call itself (same-origin), which is simpler and more reliable than hardcoding a URL. (3) Added `/app/nixpacks.toml` declaring `providers = ["node", "python"]` so Railway/Nixpacks installs both runtimes. (4) Verified end-to-end: `/` serves React SPA, `/static/js/main.*.js` returns 200, `/api/*` still routes correctly, `/api/scheduler/blocks` returns 403 (auth-required) instead of being swallowed by catch-all, deep routes like `/nonexistent/deep/route` fall back to `index.html` for React Router. APK continues to load its bundled `webDir: "build"` locally and calls `/api/*` on whatever URL it was baked with — zero impact on existing installs. Required Railway env vars going forward: `FRONTEND_URL=https://app.hetupathways.com`, `CORS_ORIGINS=https://app.hetupathways.com,https://api.hetupathways.com,https://hetupathways.com,https://heva-ops-production.up.railway.app,capacitor://localhost,http://localhost`.
