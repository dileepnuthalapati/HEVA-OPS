...
44. **POS Critical Fixes (Apr 16, 2026)** — (1) Delta Printing: OrderItem now has printed_to_kitchen flag, order updates print ONLY new items via generateDeltaKitchenReceipt(), PUT /api/orders/{id}/mark-printed marks items as printed. (2) Order Types: Takeaway/Eat In/Table selector in POS, order_type field on Order model, receipt shows bold TAKEAWAY/EAT IN/TABLE header. (3) Receipt Text Overlap: Customer receipt uses fixed 32-char monospaced formatting via formatLine() preventing price column wrap. (4) Print Settings: Admin toggles in Settings > Security for Kitchen Slip / Customer Receipt / KDS Skip Print.
