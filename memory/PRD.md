...
54. **Nav Freeze + Safe Area + Switcher Reset (Feb 11, 2026)** — User-reported bug fix batch on the live PWA. (1) **Mobile drawer auto-close on navigation** in `Sidebar.js` — converted the Sheet to a controlled component (`mobileOpen` state) and added a `useEffect` on `location.pathname` that forces the drawer closed on every route change. This was the root cause of "pages freeze and need a hard refresh when I shift modules" — the drawer's invisible overlay was lingering after a nav link was clicked inside it, blocking all subsequent pointer events. (2) **iPhone PWA safe-area fix** — added `style={{paddingTop: 'env(safe-area-inset-top)'}}` to the mobile fixed header `[data-testid=mobile-header]` so the hamburger menu no longer overlaps the iOS status bar clock/battery. (3) **Workspace switcher resets to a fixed home** — removed the `heva_workspace_last_path` localStorage behaviour and added a `defaultPath` property per workspace: POS→`/dashboard`, KDS→`/kds`, Workforce→`/workforce/shifts` (admin); staff equivalents POS→`/pos`, Workforce→`/heva-ops/shifts`. Users wanted predictable "start fresh" behaviour, not smart resume. (4) **Login brand typography** — `Login.js` now renders `Heva <span>ONE</span>` (with a space). (5) **Service worker hardening** — bumped `CACHE_NAME` to `heva-one-v5-2026-02-11`, switched hashed `/static/` bundles to cache-first (safe since filenames are hash-stamped), added `message` handler to accept `SKIP_WAITING` from the page, and added a `controllerchange` listener in `index.js` that reloads the page exactly once when a new SW takes control — users never need a hard refresh on deploy. (6) **A11y polish** — added `<SheetTitle className="sr-only">` inside the mobile drawer to satisfy Radix's DialogContent requirement and silence a console warning. Verified end-to-end by testing agent (iteration 60): 8/8 spec items pass on desktop (1920×1080) and mobile (390×844) for admin SKAdmin with POS+KDS+Workforce enabled.
